...
# Importando Dados 8n38w(08.10.2000 - 08.10.2007) ----------------------------
local = file.choose()
sst_8n38w = read.table(file = local, header=TRUE, dec=".")
sst_8n38w = sst_8n38w[,-1]
sst_8n38w$date = as.Date(sst_8n38w$date, format = "%Y-%m-%d")
attach(sst_8n38w)

teste_sst_8n38w <- ts(sst_8n38w[, 2], frequency = 365) # utilizado no teste de Dickey Fuller
...
fmla = as.formula(paste0("value ~ date + ", paste0(names(sst_8n38w)[-c(1,2)], collapse = " + ")))
...
# Modelo 14: Perceptron Multicamadas com 15 camadas ocultas (parâmetros retornados em model_arima)
...
model_tbl <- modeltime_table(
  ...
  update_modeltime_description(rep(1:26), c("ARIMA(1,1,2)",
                                            "ARIMAX(5,1,1)",
                                            "ARIMA Boost(1,1,2)",
                                            "SE(M,N,N)",
                                            "STL SE(M,N,N)",
...
calib_tbl %>%
  ...
  plot_modeltime_forecast(.interactive = FALSE,
                          .title = NULL,
                          .color_lab = "Legenda") +
  scale_x_date(limits = as.Date(c("2006-10-01","2007-10-08"), format = "%Y-%m-%d")) +
...
best_models_tbl <- modeltime_table(
  ...
  update_modeltime_description(rep(1:10),c("Prophet",
                                          "Elastic Net (λ = 0,3, α = 0,5)",
                                          "Perceptron (10 camadas)",
                                          "Perceptron (5 camadas)",
                                          "Perceptron (15 camadas)",
...
ens_calib_tbl %>% modeltime_accuracy(metric_set = metric_set(mae,
                                                             rmse))
